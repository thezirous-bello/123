...
import { computeTrailingStopUpdate, evaluateSpotExit } from "../src/spot/tradeManager.js";
import { SpotStrategyConfigSchema } from "../src/spot/schema.js";
import type { SpotPosition } from "../src/spot/repository.js";

function basePosition(overrides: Partial<SpotPosition> = {}): SpotPosition {
  return {
    id: "pos-1",
    signalId: "sig-1",
    symbol: "BTCUSDT",
...
describe("evaluateSpotExit", () => {
  it("triggers a full stop-loss exit when price drops to or below the stop", () => {
    const position = basePosition();
    const decision = evaluateSpotExit(position, new Decimal(96.5), config);
  ...
  it("does nothing between stop and TP1", () => {
    const position = basePosition();
    const decision = evaluateSpotExit(position, new Decimal(101), config);
    expect(decision).toBeNull();
  });

  it("closes tp1ClosePct at TP1 and requests a breakeven move", () => {
    const position = basePosition();
    const decision = evaluateSpotExit(position, new Decimal(102.5), config);
  ...
  it("does not re-trigger TP1 once already filled, and fires TP2 instead", () => {
    ...
    const decision = evaluateSpotExit(position, new Decimal(104.5), config);
  ...
  it("treats a breached trailing stop as a trailing_stop exit once trailing is active", () => {
    ...
    const decision = evaluateSpotExit(position, new Decimal(104.9), config);
