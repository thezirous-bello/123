...
async function resolveSymbolUniverse(mode: BybitMode): Promise<string[]> {
  ...
  const tickers = await getAllTickers(mode);
  return tickers
...
async function runSignalScan(mode: BybitMode) {
  ...
  if (!best) {
    recordLog("debug", "futures_signal_evaluation", `Scanned ${symbols.length} symbols, no Stage 1 setup qualified.`);
    return;
  }
  recordLog("info", "futures_signal_evaluation", `Stage 1 winner: ${best.symbol} (score ${best.score.toFixed(1)})`, { candidate: best });
  ...
  if (!stage2.approved) {
    recordLog("info", "futures_signal_evaluation", `Stage 2 rejected ${best.symbol}: ${stage2.blockingReasons[0]}`, { checks: stage2.checks });
    return;
  }
  ...
  recordLog("info", "futures_signal_evaluation", `Signal accepted for ${best.symbol}, attempting entry.`, { signalId: signal.id });
...
async function tryEnterSignal(signalId: string, candidate: Stage1Candidate, config: ReturnType<typeof getFuturesStrategyConfig>, mode: BybitMode, sizeMultiplier: number) {
  const instrument = await getInstrumentInfo(candidate.symbol, mode).catch(() => null);
  ...
  const leverage = Math.min(config.leverage, config.maxLeverage, instrument.maxLeverage);
  ...
  if (!risk.approved) {
    ...
    recordLog("warn", "futures_risk", `Entry blocked for ${candidate.symbol}: ${risk.blockingReasons.join("; ")}`, { checks: risk.checks });
    return;
  }
  ...
  try {
    ...
    const order = await submitMarketOrder(mode, { symbol: candidate.symbol, side: candidate.side === "long" ? "Buy" : "Sell", qty: qtyStr });
    ...
    recordLog("info", "futures_trade", `Entered ${candidate.side} ${candidate.symbol} qty=${qtyStr} @ ${candidate.entryPrice} (${leverage}x, risking $${sized.riskAmountUsd.toFixed(2)})`, {
      positionId: position.id,
    });
  } catch (err) {
...
async function manageOnePosition(position: FuturesPosition, mode: BybitMode) {
  const candles = await getKlines(position.symbol, 30, 20, mode).catch(() => null);
...
async function executeFuturesExit(
  ...
  const instrument = await getInstrumentInfo(position.symbol, mode).catch(() => null);
  const qtyStr = closeQty.toFixed(instrument ? decimalPlacesFromStep(instrument.qtyStep) : 6);
  const closeSide = position.side === "long" ? "Sell" : "Buy";

  try {
    const order = await submitMarketOrder(mode, { symbol: position.symbol, side: closeSide, qty: qtyStr, reduceOnly: true });
    ...
    recordLog("info", "futures_position", `Exit (${action}) for ${position.symbol}: closed ${qtyStr} @ ${currentPrice.toFixed()}`, { positionId: position.id });
...
export async function manualCloseFuturesPosition(positionId: string): Promise<ManualFuturesCloseResult> {
  ...
  const candles = await getKlines(position.symbol, 30, 5, position.mode).catch(() => null);
