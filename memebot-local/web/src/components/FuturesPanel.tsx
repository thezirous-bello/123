  ...
  return (
    <div className="space-y-4">
          ...
          <ConfirmModal
            ...
            body={
              <div className="space-y-2 text-sm text-white/80">
                <p>This switches the futures bot to LIVE mode. Real leveraged orders will be placed on Bybit mainnet with your real balance.</p>
                <ul className="list-disc space-y-1 pl-5 text-white/60">
                  <li>Leverage magnifies both gains and losses — a large adverse move can lose your entire margin on a position.</li>
                  <li>Liquidation can happen fast in volatile conditions, even with a stop-loss set.</li>
      ...
      >
        {!showConfig ? (
          <p className="font-mono text-xs text-white/50">
            {config.symbolUniverse === "auto" ? `Auto-scanning top ${config.autoTopNByVolume} symbols by 24h volume` : `${config.manualSymbols.length} manual symbol(s)`} · leverage{" "}
            {config.leverage}x (max {config.maxLeverage}x) · risk {config.riskPerTradePct}%/trade · max {config.maxActiveTrades} active, {config.maxPendingSignals} pending · daily loss
