import { getKlines, getOrderbook, getTicker, type Candle } from "../bybit/marketData.js";
...
// Every market-data call in this file trades against Bybit's spot order
// book, not the linear (futures) one — spot has no funding rate or open
// interest, so Stage 2's original "Funding & Open Interest" checks are
// dropped entirely here (see schema.ts's header comment for why).
 ...
 */
export async function scanSymbolStage1(symbol: string, config: SpotStrategyConfig, mode: BybitMode): Promise<Stage1Candidate | Stage1Rejection> {
  ...
  const hourlyCandles = await getKlines(symbol, 60, 60, mode, CATEGORY);
...
export async function validateSignalStage2(
  ...
  const [btc4h, btc1h, btc30m] = await Promise.all([getKlines("BTCUSDT", 240, 60, mode, CATEGORY), getKlines("BTCUSDT", 60, 220, mode, CATEGORY), getKlines("BTCUSDT", 30, 60, mode, CATEGORY)]);
  ...
  const coinCandles = await getKlines(candidate.symbol, 30, 30, mode, CATEGORY);
  ...
  // (Funding rate / open interest checks dropped — spot has neither concept,
  // those only apply to perpetual futures contracts. See the futures bot's
  // signalEngine.ts for that version of this check.)
